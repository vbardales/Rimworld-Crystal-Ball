...
to reduce, and no status tag. Illustration, exact title and summary remain unchanged.
  - unverified: `tested -> prepublished` under way through the CI. Done: `PUBLICATION.md`, the manual publish workflow (`.github/`, 49 script tests green, the `release-dry-run` and `steam-production` environments created with Virginie as reviewer), CHANGELOG 1.0.0 dated 2026-09-25, and a dry-run of `2908068077a7ed75e989bbe9837a51cbb928f8fe` as 1.0.0 with `update_description` on: run 36162739605, green, nothing sent to Steam (description 2718 bytes, sha256 `e68b8797...9ffc`; the public page of a private item is not readable, so no diff). Still open: the `steam-production` secrets (Virginie, `set-steam-secrets.sh --repo vbardales/Rimworld-Crystal-Ball`), the gallery images (`Art/WorkshopScreenshots/`, none yet; the dry-run says the folder is missing), the rollback target (no `v0.1.0` tag), and Virginie's approval of `publish`