...
# the reload, the ball where it was with the quality it had, and the giver still sending a new colonist to it. The
# balls are found again by their cell, since a reload replaces every object of the map.
#
...
Feature: a save with the ball in use
  ...
  Scenario: a save taken with two colonists gazing loads clean and the ball is still a masterwork
    ...
    And game speed is ultrafast
    When Crystal Ball: the joy giver sends "Save-3" to the ball "Kept"
    Then Crystal Ball: "Save-3" sits beside the ball "Kept"
